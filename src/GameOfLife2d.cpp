...
class GameOfLife2D : public Platform::Application {
  public:
  explicit GameOfLife2D(const Arguments& arguments);
  ...
  private:
    ...
    void update();
    PlayGround<> createPlayGroundFromArguments(const Arguments& arguments);

    Scene3D _scene;
    SceneGraph::DrawableGroup3D _drawables;
    PlayGround<Field<int>> _playground;
...
class VisualizationDrawable : public SceneGraph::Drawable3D {
  ...
  void draw(const Matrix4& transformation, SceneGraph::Camera3D& camera)
  {
    ...
    const auto center =
        Point3D<float>(-static_cast<float>(width * step / 2),
                       -static_cast<float>(heigth * step / 2), 0);
    for (size_t i = 0; i < width; ++i) {
      for (size_t j = 0; j < heigth; ++j) {
        if (const auto isAlive = _playground.getValue({i, j}); isAlive) {
          auto _transformation =
              transformation + Matrix4::translation({center.x + step * i,
                                                     center.y + step * j, 0});
...
GameOfLife2D::GameOfLife2D(const Arguments& arguments)
    : Platform::Application{arguments, Configuration{}
                                           .setTitle("Magnum Game Of Life 2D")
                                           .setSize({1200, 1000})},
      ...
      _running(false)
{
  ...
  _running = true;
  _thread = std::thread(&GameOfLife2D::update, this);
}

PlayGround<> GameOfLife2D::createPlayGroundFromArguments(
    const Arguments& arguments)
{
  ...
  if (args.value<UnsignedInt>("computemode") == 1) {
    ...
    return PlayGround<>(dimension, dimension,
                        PlayGround<>::ComputeMode::ComputeCPUSerial);
  }
  else if (args.value<UnsignedInt>("computemode") == 2) {
    auto dimension = args.value<UnsignedShort>("dimension");
    return PlayGround<>(dimension, dimension,
                        PlayGround<>::ComputeMode::ComputeCPUParallel);
  }
  else if (args.value<UnsignedInt>("computemode") == 3) {
    auto dimension = args.value<UnsignedShort>("dimension");
    return PlayGround<>(dimension, dimension,
                        PlayGround<>::ComputeMode::ComputeGPUParallel);
  }
  return PlayGround<>(32, 32, PlayGround<>::ComputeMode::ComputeCPUSerial);
}
void GameOfLife2D::drawEvent()
{
...
void GameOfLife2D::viewportEvent(ViewportEvent& event)
{
...
void GameOfLife2D::keyPressEvent(KeyEvent& event)
{
  switch (event.key()) {
    case Key::L:
...
void GameOfLife2D::pointerPressEvent(PointerEvent& event)
{
  if (!event.isPrimary() ||
...
void GameOfLife2D::pointerReleaseEvent(PointerEvent& event)
{
  if (!event.isPrimary() ||
      !(event.pointer() & (Pointer::MouseLeft | Pointer::Finger)))
    return;
...
void GameOfLife2D::pointerMoveEvent(PointerMoveEvent& event)
{
  if (!event.isPrimary() ||
...
void GameOfLife2D::scrollEvent(ScrollEvent& event)
{
...
MAGNUM_APPLICATION_MAIN(Magnum::Examples::GameOfLife2D)
