...
// Integrate Thread:
// Poolhttps://medium.com/@bhushanrane1992/getting-started-with-c-thread-pool-b6d1102da99a
          ...
          this->mpool.ExecuteTask([indexz = z, this, width, heigth]() {
            for (size_t x = 1; x < width - 1; ++x) {
              for (size_t y = 1; y < heigth - 1; ++y) {
                size_t neighbours = 0;
                neighbours +=
                    this->mcurrentField.getNeighbours({x, y, indexz}, -1);
                neighbours +=
                    this->mcurrentField.getNeighbours({x, y, indexz}, 0);
                neighbours +=
                    this->mcurrentField.getNeighbours({x, y, indexz}, +1);
                // check rule
